...
def tensor_reduce(fn: Callable[[float, float], float]) -> Any:
    ...
    * `out_shape` will be the same as `a_shape`
    ...
    def _reduce(
        ...
        # TODO: Implement for Task 2.3.
        # print("reduce")
        reduce_shape1 = [1] * len(out_shape)

        reduce_shape1[reduce_dim] = a_shape[reduce_dim]
        reduce_size = np.prod(reduce_shape1)

        out_index = np.zeros(MAX_DIMS, np.int32)
        a_index = np.zeros(MAX_DIMS, np.int32)
        out_shape = np.asarray(out_shape, dtype=np.int32)
        reduce_shape = np.array(reduce_shape1)

        for i in range(len(out)):
            # print(type(out_shape), type(out_index))
            to_index(i, out_shape, out_index)
